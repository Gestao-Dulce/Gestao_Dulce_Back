...
function imprimir({
  ...
  const th = colunas.map((c) => `<th style="text-align:${c.align ?? "left"}">${c.label}</th>`).join("");
  const body = linhas.length ? linhas.map(
    (r) => `<tr>${r.map(
      (cell, i) => `<td style="text-align:${colunas[i]?.align ?? "left"}">${cell ?? "—"}</td>`
    ).join("")}</tr>`
  ).join("") : `<tr><td colspan="${colunas.length}" style="text-align:center;padding:24px;color:#888">Sem registros.</td></tr>`;
  const tfoot = rodape ? `<tfoot><tr>${rodape.map(
    (cell, i) => `<td style="text-align:${colunas[i]?.align ?? "left"}">${cell ?? ""}</td>`
  ).join("")}</tr></tfoot>` : "";
  ...
  <style>
    ...
    <div class="meta">Emitido em ${(/* @__PURE__ */ new Date()).toLocaleString("pt-BR")}${meta ? ` • ${meta}` : ""} • ${linhas.length} registro(s)</div>
